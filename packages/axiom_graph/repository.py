...
class GraphRepository:
    ...
    async def create_query(self, text: str, job_id: str | None = None) -> str:
        ...
        async with self._driver.session() as session:
            await session.run(cypher, id=qid, text=text, job_id=job_id or "", created_at=_now())
        return qid
    ...
    async def get_axiom(self, axiom_id: str) -> dict[str, Any] | None:
        ...
        async with self._driver.session() as session:
            ...
            return None if record is None else dict(record["a"])
    ...
    async def get_query(self, query_id: str) -> dict[str, Any] | None:
        ...
        async with self._driver.session() as session:
            ...
            record = await result.single()
            return None if record is None else dict(record["q"])
